...
const ProfileScreen: React.FC = () => {
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      try {
        const chercheurData = await getChercheurById(CURRENT_USER_ID);
        ...
        if (chercheurData && chercheurData.id_institution) {
          ...
          if (foundInstitution) {
            setInstitutionName(foundInstitution.nom);
          } else {
            setInstitutionName('Institution inconnue');
          }
        } else {
          setInstitutionName('Non spécifiée');
        }
      } catch (err) {
        console.error('Erreur lors de la récupération du profil ou de l\'institution:', err);
        setError('Impossible de charger le profil ou les détails de l\'institution. Veuillez vérifier la connexion.');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (error) {
    return (
      <View style={styles.errorContainer}>
        ...
        <TouchableOpacity style={styles.retryButton} onPress={() => { setLoading(true); setError(null); navigation.replace('Profile'); }}>
    ...
    <ScrollView style={styles.container}>
      ...
      <View style={styles.infoSection}>
        ...
        <View style={styles.infoRow}>
          ...
          <Text style={styles.infoText}>Institution: {institutionName || 'Chargement...'}</Text>
        </View>
        <View style={styles.infoRow}>
